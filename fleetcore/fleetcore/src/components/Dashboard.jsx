...
function DeadlineSection({ title, icon, color, type, vehicles, events }) {
  ...
  return (
    <div style={{ background: "#0d1424", border: "1px solid " + color + "44", borderTop: "3px solid " + color, borderRadius: 12, padding: 20, flex: 1, minWidth: 280 }}>
      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {items.map(({ vehicle, nextDate, days }) => (
            <div key={vehicle.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#080d16", borderRadius: 8, padding: "8px 12px", borderLeft: "3px solid " + urgencyColor(days) }}>
              <div>
                <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700, color: "#3B82F6", fontSize: 13 }}>{vehicle.plate}</span>
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "#475569", fontSize: 14, marginTop: 4 }}>{vehicles.length} veicoli · {totalEvents} eventi</p>
        </div>
        {vehicles.length === 0 && (
          <button onClick={onAddVehicle} style={{ background: "#3B82F6", border: "none", color: "#fff", padding: "10px 20px", borderRadius: 8, fontSize: 14, fontWeight: 600 }}>+ Aggiungi Veicolo</button>
        )}
      </div>

      {/* SEARCH BAR */}
      <div style={{ position: "relative", marginBottom: 28 }}>
        <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", fontSize: 16, pointerEvents: "none", color: "#334155" }}>🔍</span>
        <input
          value={search}
          onChange={e => setSearch(e.target.value)}
          placeholder="Cerca per targa, tipo evento (tagliando, lavaggio…), data (15/05/2024)…"
          style={{
            width: "100%", background: "#0d1424", border: "1px solid #243044",
            borderRadius: 12, color: "#f1f5f9", padding: "13px 44px 13px 44px",
        ...
        />
        {search && (
          <button onClick={() => setSearch("")} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "#1a2538", border: "none", color: "#94a3b8", borderRadius: 6, cursor: "pointer", fontSize: 13, padding: "3px 8px" }}>✕ Cancella</button>
        )}

        {/* RESULTS */}
        {q.length >= 2 && (
          <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, zIndex: 100, background: "#0d1424", border: "1px solid #243044", borderRadius: 12, boxShadow: "0 12px 40px rgba(0,0,0,.6)", overflow: "hidden" }}>
            {searchResults.length === 0 ? (
              <div style={{ padding: "20px", textAlign: "center" }}>
                <p style={{ color: "#475569", fontSize: 14 }}>Nessun risultato per "<strong style={{ color: "#94a3b8" }}>{search}</strong>"</p>
              </div>
            ) : (
              <>
                <div style={{ padding: "10px 16px 6px", borderBottom: "1px solid #1a2538" }}>
                  <p style={{ fontSize: 11, color: "#334155", fontWeight: 700, letterSpacing: "0.08em" }}>
                ...
                {searchResults.map((r, i) => (
                  ...
                  >
                    <span style={{ fontSize: 20, flexShrink: 0, width: 32, height: 32, background: "#1a2538", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>{r.icon}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700, fontSize: 13, color: "#3B82F6", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.label}</p>
                      <p style={{ fontSize: 12, color: "#475569", margin: 0, marginTop: 2 }}>{r.sub}</p>
                    </div>
                    <span style={{ color: "#243044", fontSize: 16, flexShrink: 0 }}>→</span>
                  </button>
                ))}
              </>
            )}
        ...
        <section style={{ marginBottom: 36 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 14 }}>
            {allDeadlines.map((d, i) => (
              <div key={i} onClick={() => onVehicleClick(d.vehicle.id)} style={{ background: "#0d1424", border: "1px solid #1a2538", borderLeft: "3px solid " + urgencyColor(d.days), borderRadius: 12, padding: "14px 16px", cursor: "pointer", transition: "transform .15s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <span style={{ fontFamily: "'DM Mono', monospace", fontWeight: 700, color: "#3B82F6", fontSize: 14 }}>{d.vehicle.plate}</span>
        ...
        <section>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {recentEvents.map(ev => {
              ...
              return (
                <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 14, background: "#0d1424", border: "1px solid #1a2538", borderRadius: 10, padding: "12px 16px" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      {vehicle && <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#475569" }}>{vehicle.plate}</span>}
                    </div>
                    {ev.notes && <p style={{ color: "#475569", fontSize: 12, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ev.notes}</p>}
                  </div>
                  <span style={{ color: "#334155", fontSize: 12, fontFamily: "'DM Mono', monospace", flexShrink: 0 }}>{formatDate(ev.date)}</span>
                </div>
              );
            })}
          </div>
